...
(function () {
    ...
    /** Build slowed + reverb version from mastered blob URL. Sets state.slowedReverbUrl.
     * Reverb is always applied here (JOSH_VERB_DISABLED does not apply to this feature).
     * opts: { playbackRate, reverbMix, reverbDecaySeconds, shimmerDb } - all optional, from state if not set.
     */
    async function buildSlowedReverbFromMasterUrl(masteredUrl, opts) {
        if (!masteredUrl || !state) return;
        const playbackRate = opts && typeof opts.playbackRate === 'number' ? opts.playbackRate : (state.slowedPlaybackRate != null ? state.slowedPlaybackRate : 0.8);
        const reverbMix = opts && typeof opts.reverbMix === 'number' ? opts.reverbMix : (state.slowedReverbMix != null ? state.slowedReverbMix : 0.4);
        const reverbDecaySeconds = opts && typeof opts.reverbDecaySeconds === 'number' ? opts.reverbDecaySeconds : (state.slowedReverbDecaySeconds != null ? state.slowedReverbDecaySeconds : 0.45);
        const shimmerDb = opts && typeof opts.shimmerDb === 'number' ? opts.shimmerDb : (state.slowedShimmerDb != null ? state.slowedShimmerDb : 0);
        state.slowedPlaybackRate = playbackRate;
        state.slowedReverbMix = reverbMix;
        state.slowedReverbDecaySeconds = reverbDecaySeconds;
        state.slowedShimmerDb = shimmerDb;
        const res = await fetch(masteredUrl);
        ...
        const slowedLength = Math.ceil(originalLength / playbackRate);
        const offlineCtx = new (window.OfflineAudioContext || window.webkitOfflineAudioContext)(numChannels, slowedLength, sr);
        const src = offlineCtx.createBufferSource();
        src.buffer = buffer;
        src.playbackRate.value = playbackRate;
        const dryGain = offlineCtx.createGain();
        dryGain.gain.value = 1;
        const wetAmount = Math.max(0.05, Math.min(0.9, reverbMix * 0.65));
        const wetGain = offlineCtx.createGain();
        wetGain.gain.value = wetAmount;
        const plate = createAlgorithmicPlate(offlineCtx, sr, reverbDecaySeconds, 0.5);
        const sumGain = offlineCtx.createGain();
        sumGain.gain.value = 1 / (1 + wetAmount);
        src.connect(dryGain);
        src.connect(plate.input);
        plate.output.connect(wetGain);
        dryGain.connect(sumGain);
        wetGain.connect(sumGain);
        ...
        if (shimmerDb > 0) {
            ...
            highShelf.gain.value = Math.min(9, shimmerDb);
        ...
        if (state.slowedReverbUrl) {
            URL.revokeObjectURL(state.slowedReverbUrl);
        }
        state.slowedReverbUrl = URL.createObjectURL(blob);
    }
    ...
    window.MegaMix.buildSlowedReverbFromMasterUrl = buildSlowedReverbFromMasterUrl;
