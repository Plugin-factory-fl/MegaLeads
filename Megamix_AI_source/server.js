...
// Temporary separation inputs: jobId -> { path, createdAt }. Cleaned after use or after 10 min.
const separateInputStore = new Map();
const SEPARATE_INPUT_MAX_AGE_MS = 10 * 60 * 1000;
function cleanupSeparateInputs() {
  const now = Date.now();
  for (const [id, entry] of separateInputStore.entries()) {
    if (now - entry.createdAt > SEPARATE_INPUT_MAX_AGE_MS) {
      try { fs.unlinkSync(entry.path); } catch (_) {}
      separateInputStore.delete(id);
    }
  }
}
setInterval(cleanupSeparateInputs, 60 * 1000);

const BASE_URL = process.env.BASE_URL || 'https://megamixai-mvp-backend.onrender.com';
...
// One-time URL for Replicate to fetch the uploaded audio (no auth)
app.get('/api/separate-input/:id', (req, res) => {
  const entry = separateInputStore.get(req.params.id);
  if (!entry || !fs.existsSync(entry.path)) {
    return res.status(404).send('Not found');
  }
  const ext = path.extname(entry.path).toLowerCase();
  res.type(ext === '.wav' ? 'audio/wav' : 'audio/mpeg');
  res.sendFile(entry.path, (err) => {
    try { fs.unlinkSync(entry.path); } catch (_) {}
    separateInputStore.delete(req.params.id);
  });
});

// Extract subscription ID from invoice (handles both legacy and 2025+ Stripe API structures)
function getSubscriptionIdFromInvoice(invoice) {
  // New API (2025+): invoice.parent.subscription_details.subscription
  const fromParent = invoice?.parent?.subscription_details?.subscription;
  if (fromParent) return typeof fromParent === 'string' ? fromParent : fromParent?.id;

  // Legacy: invoice.subscription (string or expanded object)
  const fromTop = invoice?.subscription;
  if (!fromTop) return null;
  return typeof fromTop === 'string' ? fromTop : fromTop?.id;
}
...
app.post('/webhooks/stripe', express.raw({type: 'application/json'}), async (req, res) => {
  ...
  switch (event.type) {
    ...
    case 'invoice.payment_succeeded':
      ...
      const subIdSucceeded = getSubscriptionIdFromInvoice(paymentSucceededSession);
      if (subIdSucceeded) {
        try {
          const subscription = await stripe.subscriptions.retrieve(subIdSucceeded);
          ...
          const result = await pool.query(
            ...
            [expiresAt, subIdSucceeded]
          );
          if (result.rowCount === 0) {
            console.warn(`[invoice.payment_succeeded] No license found for subscription ${subIdSucceeded}`);
          } else {
            console.log(`Updated license status to active for subscription ${subIdSucceeded}`);
          }
        } catch (error) {
          console.error('Error updating license on payment success:', error);
        }
      } else {
        console.warn('[invoice.payment_succeeded] Could not extract subscription ID from invoice', paymentSucceededSession.id);
      }
      break;
    ...
    case 'invoice.payment_failed':
      ...
      const subIdFailed = getSubscriptionIdFromInvoice(paymentFailedSession);
      if (subIdFailed) {
        try {
          // Check if subscription will be cancelled or is past due
          const subscription = await stripe.subscriptions.retrieve(subIdFailed);
          ...
          const result = await pool.query(
            `UPDATE licenses 
             SET status = $1
             WHERE stripe_subscription_id = $2`,
            [newStatus, subIdFailed]
          );
          if (result.rowCount === 0) {
            console.warn(`[invoice.payment_failed] No license found for subscription ${subIdFailed}`);
          } else {
            console.log(`Updated license status to ${newStatus} for subscription ${subIdFailed}`);
          }
        } catch (error) {
          console.error('Error updating license on payment failure:', error);
        }
      } else {
        console.warn('[invoice.payment_failed] Could not extract subscription ID from invoice', paymentFailedSession.id);
      }
      break;
...
// One-file stem separation (subscribers only). Uses Replicate Demucs, 4 stems only: Vocals, Drums, Bass, Other.
const DEMUCS_MODEL = 'ryan5453/demucs:b26a4313b4d75983d60657f80dfa93b9beb354f6e4fa29ecd27ffe14d60117f6';
const STEM_NAMES = ['Vocals', 'Drums', 'Bass', 'Other'];
const STEM_SEPARATE_DURATIONS_MAX = 50;
const STEM_SEPARATE_DEFAULT_ESTIMATE_SEC = 120;
const stemSeparateDurations = [];

app.post('/api/separate', requireSubscriber, uploadMem.single('audio'), async (req, res) => {
  const jobId = `sep_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
  ...
  try {
    ...
    if (!process.env.REPLICATE_API_TOKEN) {
      return res.status(503).json({ error: 'Stem separation is not configured. Set REPLICATE_API_TOKEN.' });
    }

    const modelVariant = 'htdemucs';
    const stemNames = STEM_NAMES;
    const maxStems = 4;

    const ext = (req.file.originalname || '').toLowerCase().endsWith('.wav') ? '.wav' : '.mp3';
    inputPath = path.join(tmpDir, `${jobId}${ext}`);
    fs.writeFileSync(inputPath, req.file.buffer);
    separateInputStore.set(jobId, { path: inputPath, createdAt: Date.now() });

    const audioUrl = `${BASE_URL}/api/separate-input/${jobId}`;
    console.log('[api/separate] audioUrl for Replicate:', audioUrl, 'model:', modelVariant);
    const replicate = new Replicate({ auth: process.env.REPLICATE_API_TOKEN, useFileOutput: false });
    const rawOutput = await replicate.run(DEMUCS_MODEL, { input: { audio: audioUrl, model: modelVariant } });
    // Replicate client v1.x may return output directly or wrapped; handle both
    const output = rawOutput && typeof rawOutput === 'object' && rawOutput.output !== undefined
      ? rawOutput.output
      : rawOutput;
    console.log('[api/separate] Replicate output keys:', output && typeof output === 'object' ? Object.keys(output) : typeof output);
    if (output && output.stems && output.stems[0]) {
      const first = output.stems[0];
      console.log('[api/separate] first stem keys:', typeof first === 'object' ? Object.keys(first) : typeof first, 'audio type:', typeof first.audio, 'audio.url:', first.audio && first.audio.url);
    }

    function getStemUrl(item) {
      if (!item) return null;
      ...
      if (typeof item === 'object') {
        ...
        if (audio && typeof audio === 'object') {
          ...
          if (typeof audio.url === 'function') {
            try {
              const u = audio.url();
              return u ? (u.href || String(u)) : null;
            } catch (_) { return null; }
          }
        }
    ...
    const stemItems = output && Array.isArray(output.stems)
      ? output.stems
      : Array.isArray(output)
        ? output
        : output && Array.isArray(output.separation)
          ? output.separation
          : [];
    const stemUrls = [];
    const stemApiNames = [];
    for (let i = 0; i < Math.min(stemItems.length, maxStems); i++) {
      const item = stemItems[i];
      const u = getStemUrl(item);
      if (u) {
        stemUrls.push(u);
        const apiName = item && typeof item === 'object' && typeof item.name === 'string' ? item.name : null;
        stemApiNames.push(apiName);
      }
    }

    const stems = [];
    for (let i = 0; i < Math.min(stemUrls.length, maxStems); i++) {
      const url = stemUrls[i];
      if (!url || typeof url !== 'string') continue;
      const resp = await fetch(url);
      const buf = Buffer.from(await resp.arrayBuffer());
      const displayName = stemApiNames[i]
        ? (stemApiNames[i].charAt(0).toUpperCase() + stemApiNames[i].slice(1).toLowerCase())
        : (stemNames[i] || `Stem ${i + 1}`);
      stems.push({ name: displayName, data: buf.toString('base64') });
    }

    try { fs.unlinkSync(inputPath); } catch (_) {}
    separateInputStore.delete(jobId);

    if (stems.length === 0) {
      return res.status(500).json({ error: 'Stem separation produced no output.' });
    }
    const durationSec = (Date.now() - startTime) / 1000;
    stemSeparateDurations.push(durationSec);
    if (stemSeparateDurations.length > STEM_SEPARATE_DURATIONS_MAX) stemSeparateDurations.shift();
    const avgSec = stemSeparateDurations.length
      ? stemSeparateDurations.reduce((a, b) => a + b, 0) / stemSeparateDurations.length
      : STEM_SEPARATE_DEFAULT_ESTIMATE_SEC;
    const estimatedSecondsForNext = Math.round(avgSec);
    res.json({ stems, estimatedSecondsForNext });
  } catch (err) {
    try {
...
app.get('/api/separate-estimate', (req, res) => {
  const avgSec = stemSeparateDurations.length
    ? stemSeparateDurations.reduce((a, b) => a + b, 0) / stemSeparateDurations.length
    : STEM_SEPARATE_DEFAULT_ESTIMATE_SEC;
  res.json({ estimatedSeconds: Math.round(avgSec) });
});
    ...
    res.send(`
      <html>
        <head>
          ...
          <!-- Google tag (gtag.js) -->
          <script async src="https://www.googletagmanager.com/gtag/js?id=G-4JZ3RNEJWY"></script>
          <script>
            window.dataLayer = window.dataLayer || [];
            function gtag(){dataLayer.push(arguments);}
            gtag('js', new Date());
            gtag('config', 'G-4JZ3RNEJWY');
            gtag('event', 'free_trial_complete', {
              event_category: 'checkout',
              event_label: 'license_created',
              value: 1,
          ...
          <style>
            ...
            .keep-in-touch h3 { color: #10b981; margin: 0 0 12px 0; font-size: 1.2rem; }
            .keep-in-touch p { margin: 0 0 16px 0; opacity: 0.9; font-size: 0.95rem; }
            .keep-in-touch form { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; justify-content: center; }
            .keep-in-touch input[type="email"] {
              padding: 10px 16px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2);
              background: rgba(0,0,0,0.2); color: white; font-size: 1rem; min-width: 220px;
            }
            .keep-in-touch .btn-keep-touch {
              background: linear-gradient(135deg, #10b981 0%, #059669 100%);
              color: white; border: none; padding: 10px 20px; border-radius: 8px;
              font-size: 0.95rem; font-weight: bold; cursor: pointer;
            }
            .keep-in-touch .btn-keep-touch:disabled { opacity: 0.7; cursor: not-allowed; }
            .keep-in-touch .success-msg { color: #10b981; margin-top: 12px; font-weight: 600; }
        ...
        <body>
          <div class="container">
            ...
            <div class="keep-in-touch" id="keepInTouchBlock">
              <h3>Stay in the loop</h3>
              <p>Get product updates and tips. No spam—just the good stuff.</p>
              <form id="keepInTouchForm" onsubmit="return submitKeepInTouch(event);">
                <input type="email" id="keepInTouchEmail" placeholder="your@email.com" required>
                <button type="submit" class="btn-keep-touch" id="keepInTouchBtn">Keep me in touch</button>
              </form>
              <p class="success-msg hidden" id="keepInTouchSuccess">You're on the list!</p>
            </div>
          ...
          <script>
            ...
            function submitKeepInTouch(e) {
              e.preventDefault();
              var email = document.getElementById('keepInTouchEmail').value.trim();
              var btn = document.getElementById('keepInTouchBtn');
              var successEl = document.getElementById('keepInTouchSuccess');
              if (!email || !/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(email)) return false;
              btn.disabled = true;
              fetch('/mailchimp-signup', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: email, signupSource: 'buyer-success-page' })
              }).then(function(r) { return r.json(); }).then(function(data) {
                if (data.success) {
                  successEl.classList.remove('hidden');
                  document.getElementById('keepInTouchForm').style.display = 'none';
                }
...
// Subscription lookup by license key only (for Manage Subscription page).
// License key uniquely identifies one subscription; email can be tied to multiple.
async function lookupSubscriptionByLicenseKey(licenseKey) {
  const trimKey = typeof licenseKey === 'string' ? licenseKey.trim() : '';
  if (!trimKey) return { error: 'License key is required' };
  const normalized = normalizeLicenseKey(trimKey);
  if (!normalized || normalized.length < 16) return { error: 'Invalid license key format' };
  const result = await pool.query(
    `SELECT * FROM licenses WHERE UPPER(REPLACE(REPLACE(REPLACE(license_key, ' ', ''), '-', ''), '_', '')) = $1`,
    [normalized]
  );
  if (!result.rows.length) return { error: 'No subscription found for this license key' };
  const license = result.rows[0];
  if (license.status !== 'active') return { error: 'This subscription is not active' };
  if (new Date() > new Date(license.expires_at)) return { error: 'This subscription has expired' };
  ...
  if (license.stripe_subscription_id) {
    try {
      ...
      hasActiveSubscription = ['active', 'trialing'].includes(sub.status);
      if (!hasActiveSubscription) stripeCustomerId = null;
    } catch (e) {
      stripeCustomerId = null;
    }
  }
  return { license, hasActiveSubscription, stripeCustomerId };
}

app.post('/api/subscription/lookup', async (req, res) => {
  try {
    const { licenseKey } = req.body || {};
    const out = await lookupSubscriptionByLicenseKey(licenseKey);
    if (out.error) {
      return res.status(400).json({ ok: false, error: out.error });
    }
    res.json({
      ok: true,
      hasActiveSubscription: out.hasActiveSubscription,
      stripeCustomerId: out.stripeCustomerId || undefined
    });
  } catch (err) {
    console.error('[api/subscription/lookup]', err.message || err);
...
app.post('/api/subscription/portal', async (req, res) => {
  try {
    ...
    if (!out.stripeCustomerId) {
      return res.status(400).json({ error: 'No billing portal available for this subscription' });
    }
    const baseUrl = process.env.BASE_URL || 'https://megamixai-mvp-backend.onrender.com';
    const session = await stripe.billingPortal.sessions.create({
      customer: out.stripeCustomerId,
      return_url: baseUrl + '/manage'
    });
    res.json({ url: session.url });
  } catch (err) {
    console.error('[api/subscription/portal]', err.message || err);
    res.status(500).json({ error: err.message || 'Could not open billing portal' });
  }
});

const managePageHtml = `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Manage Subscription – MegaMix AI</title>
  <link rel="icon" type="image/png" href="/assets/Logo.png">
  <style>
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #1a1a1a; color: #fff; margin: 0; padding: 40px 20px; min-height: 100vh; box-sizing: border-box; }
    .container { max-width: 480px; margin: 0 auto; }
    h1 { color: #8b5cf6; font-size: 1.75rem; margin-bottom: 8px; }
    .sub { color: #94a3b8; font-size: 0.95rem; margin-bottom: 24px; }
    label { display: block; margin-bottom: 8px; font-size: 0.95rem; }
    input[type="text"] { width: 100%; padding: 12px 14px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: rgba(255,255,255,0.08); color: #fff; font-size: 1rem; margin-bottom: 16px; box-sizing: border-box; }
    .btn { display: inline-block; padding: 12px 24px; border-radius: 8px; font-size: 1rem; font-weight: 600; cursor: pointer; border: none; text-align: center; }
    .btn-verify { background: linear-gradient(135deg, #8b5cf6 0%, #a855f7 100%); color: #fff; }
    .btn-verify:hover { opacity: 0.9; }
    .btn-upgrade { background: #555; color: #999; cursor: not-allowed; opacity: 0.7; margin-top: 8px; }
    .btn-cancel { background: #dc2626; color: #fff; margin-top: 16px; width: 100%; }
    .btn-cancel:hover { background: #b91c1c; }
    .actions { margin-top: 24px; padding-top: 24px; border-top: 1px solid rgba(255,255,255,0.1); display: none; }
    .actions.visible { display: block; }
    .error { color: #f87171; font-size: 0.9rem; margin-top: 8px; }
    .back { margin-top: 32px; }
    .back a { color: #10b981; text-decoration: none; }
  </style>
</head>
<body>
  <div class="container">
    <h1>Manage Subscription</h1>
    <p class="sub">Enter your license key to manage or cancel your subscription.</p>
    <label for="manage-input">License key</label>
    <input type="text" id="manage-input" placeholder="XXXX-XXXX-XXXX-XXXX" autocomplete="off">
    <button type="button" class="btn btn-verify" id="manage-verify">Verify</button>
    <p id="manage-error" class="error" style="display:none;"></p>
    <div id="manage-actions" class="actions">
      <button type="button" class="btn btn-upgrade" disabled>Upgrade to MegaMix PRO — $9.99/mo (Coming soon)</button>
      <button type="button" class="btn btn-cancel" id="manage-cancel">Cancel subscription</button>
    </div>
    <div class="back"><a href="/">← Back to Home</a></div>
  </div>
  <script>
    var verifiedLicenseKey = null;
    document.getElementById('manage-verify').onclick = function() {
      var input = document.getElementById('manage-input');
      var err = document.getElementById('manage-error');
      var actions = document.getElementById('manage-actions');
      var v = (input && input.value) ? input.value.trim() : '';
      if (!v) { err.textContent = 'Please enter your license key.'; err.style.display = 'block'; return; }
      err.style.display = 'none';
      fetch('/api/subscription/lookup', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ licenseKey: v }) })
        .then(function(r) { return r.json(); })
        .then(function(data) {
          if (data.ok) {
            verifiedLicenseKey = v;
        ...
        .catch(function() { err.textContent = 'Something went wrong. Try again.'; err.style.display = 'block'; });
    };
    document.getElementById('manage-cancel').onclick = function() {
      if (!verifiedLicenseKey) return;
      fetch('/api/subscription/portal', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ licenseKey: verifiedLicenseKey }) })
        .then(function(r) { return r.json(); })
        .then(function(data) {
...
// Build Mailchimp tags array: signupSource (buyer/prospective) + optional format/platform
function buildMailchimpTags(signupSource, format, platform) {
  const tags = [];
  if (signupSource && typeof signupSource === 'string') {
    tags.push({ name: signupSource.trim(), status: 'active' });
  }
  if (format && platform) {
    tags.push({ name: `${format}-${platform}`, status: 'active' });
  }
  return tags;
}
...
app.post('/mailchimp-signup', async (req, res) => {
  try {
    const { email, format, platform, signupSource, source } = req.body;
    ...
    const effectiveSource = signupSource || source;
    const tags = buildMailchimpTags(effectiveSource, format, platform);
    ...
    console.log('Mailchimp signup successful:', { email, format, platform, signupSource: effectiveSource, mailchimpId: response.id });
...
app.post('/contact-support', async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: smtpUser || email,
      to: 'megamix.ai.plugins@gmail.com',
    ...
    console.log(`Contact form submission sent from ${email} to megamix.ai.plugins@gmail.com`);
